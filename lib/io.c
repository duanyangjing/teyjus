...
/* unify types */
static void typesUnify(DF_TypePtr typ1, DF_TypePtr typ2)
{
    AM_pdlError(2);
    AM_initTypesPDL();
...
static WordPtr getStreamFromTerm(DF_TermPtr tmPtr)
{
    int cstInd;
    HN_hnorm(tmPtr);
    tmPtr = DF_termDeref(tmPtr);
...
static void bindVarToStream(DF_TermPtr varPtr, WordPtr stream)
{   MemPtr nhreg;
...
static void bindVarToString(DF_TermPtr varPtr, char* str)
{
...
static void closeStreamTerm(DF_TermPtr tmPtr)
{
    WordPtr stream;
...
static void doOpen(char *inMode)
{
  WordPtr stream;
  char    *fname;

  fname = getStringFromTerm((DF_TermPtr)AM_reg(1));  
  ...
  bindVarToStream(((DF_TermPtr)AM_reg(2)), stream);
  AM_preg = AM_cpreg;
}

static void doClose()
{
  closeStreamTerm((DF_TermPtr)(AM_reg(1)));
  AM_preg = AM_cpreg;
}
...
void openIn()       { doOpen(STREAM_READ);   }
...
/* type  open_out, open_append   string -> out_stream -> o
   ...
   writing at the end.                          */
void openOut()      { doOpen(STREAM_WRITE);  }
void openApp()      { doOpen(STREAM_APPEND); }

/* type close_in    in_stream -> o
   close the stream                             */
void closeIn()      { doClose();             }
void closeOut()     { doClose();             }
...
void input()
{
    ...
    num = getIntegerFromTerm((DF_TermPtr)AM_reg(2)) + 1; /* +1 for the null
                                                                 character */
    if (num <= 0) EM_error(BI_ERROR_NEGATIVE_VALUE, num);
    
    stream = getStreamFromTerm((DF_TermPtr)AM_reg(1));
...
void output()
{
  ...
  stream = getStreamFromTerm((DF_TermPtr)AM_reg(1));
  str    = getStringFromTerm((DF_TermPtr)AM_reg(2));
...
void print()
{
    char *str = getStringFromTerm((DF_TermPtr)AM_reg(1));
...
{   
  ...
  typesUnify(typ, (DF_TypePtr)(AM_reg(3)));
...
#define  MAX_LINE_LENGTH 1024
  ...
  typesUnify(typ, (DF_TypePtr)(AM_reg(2)));
...
void termToStr()
{
    ...
    if (str) bindVarToString((DF_TermPtr)AM_reg(2), str);
    else bindVarToString((DF_TermPtr)AM_reg(2), "");
